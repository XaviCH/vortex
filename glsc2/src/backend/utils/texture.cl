/*
    Utility functions of textures for multiple configurations and device support,
    enabling a common API for kernels that uses textures
*/
...
// VARIABLE wrappers
    ...
    #define TEXTURE_UNIT_PARAM(_NAME) \
        ro_texture2d_t _NAME##_image, \
        sampler_t _NAME##_sampler
#else
    #define TEXTURE_UNIT_PARAM(_NAME) \
        ro_texture2d_t _NAME##_image
#endif
...
static inline float2 get_wrapped_ncoord(float2 coord, const texture_data_t texture_data)
{
    ...
    {
        ncoord.x = coord.x - floor(coord.x);
    }
    ...
    {
        ncoord.y = coord.y - floor(coord.y);
    }
    ...
    if (get_texture_data_require_diff_x(texture_data, coord.x)) ncoord.x = - ncoord.x;
    if (get_texture_data_require_diff_y(texture_data, coord.y)) ncoord.y = - ncoord.y;
...
static inline float4 read_2d_texturef(
    ...
    // GL Texture Wrapping and CL Image Addressing do not have a one to one mapping,
    // this code section enables a complete mapping between both.
    #ifdef DEVICE_IMAGE_ENABLED
    {
        if (get_texture_data_require_clamp_x(texture_data))
        {
    ...
    #else
    // software addressing is done at read_2d_bufferf
    #endif
    ...
    {
        #ifdef DEVICE_IMAGE_ENABLED
            return read_imagef_gl(TEXTURE_UNIT_ARG(texture), coord, round(lod));
        #else
    ...
    #ifdef DEVICE_IMAGE_ENABLED
    {
        color_0 = read_imagef_gl(TEXTURE_UNIT_ARG(texture), coord, lod1);
        color_1 = read_imagef_gl(TEXTURE_UNIT_ARG(texture), coord, lod2);
    }
    #else
    {
...
static inline float4 read_imagef_gl(
    ...
    return uint4_to_float4_color(TEXTURE_UNIT_ARG(texture), coord, lod)
