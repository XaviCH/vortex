...
static uint32_t __device_orch_is_deferred_clear(clear_state_t state)
{
...
{
    if (framebuffer->context_id == -1) return NULL;

    return &orch->contexts[framebuffer->context_id];
}
...
static void __orch_flush_draw_state(orch_handler_t* orch, orch_framebuffer_handler_t* framebuffer)
{
    ...
    uint32_t deferred_clear = __device_orch_is_deferred_clear(framebuffer->clear_state);
...
{
    return orch->context_framebuffer_attachments[context_id] < DEVICE_CONTEXT_NUMBER;
}
*/
...
static void __orch_attach_new_context(orch_handler_t* orch, orch_framebuffer_handler_t* framebuffer)
{
...
static void __orch_draw_vertices(
    ...
    if (
        context == NULL ||
        __orch_require_flush_context(framebuffer, shader_id, mode, num_vertices)
    )
    {
        __orch_attach_new_context(orch, framebuffer);
        context = __orch_get_attached_context(orch, framebuffer);
    }
...
static void __orch_flush(orch_handler_t* orch, orch_framebuffer_handler_t* framebuffer)
{
...
static void orch_init(orch_handler_t* orch)
{
    *orch = {
    ...
    for(size_t context_id; context_id < DEVICE_CONTEXT_NUMBER; ++context_id)
    {
...
{
    orch_framebuffer_handler_t* framebuffer = &orch->framebuffers[framebuffer_id];
...
static void orch_attach_vertex_uniform(
    orch_handler_t* orch, 
    ...
    device_load_vertex_uniform(context, uniform_data);

    device_bind_vertex_uniform(context);
}

static void orch_attach_vertex_fragment_uniform(
    orch_handler_t* orch,
    size_t framebuffer_id
) {
    orch_framebuffer_handler_t* framebuffer = &orch->framebuffers[framebuffer_id];
    ...
    device_bind_vertex_fragment_uniform(context);
}

// Upload functions

static void orch_upload_vertex_attributes(
    orch_handler_t* orch, 
    size_t framebuffer_id, 
    ...
    device_load_vertex_attributes(context, vertex_attribs);
}

static void orch_upload_vertex_attribute_data(
    orch_handler_t* orch, 
    size_t framebuffer_id, 
    ...
    device_load_vertex_attribute_data(context, data);
}

static void orch_upload_fragment_data(
    orch_handler_t* orch, 
    size_t framebuffer_id, 
    void* uniform_data,
    rop_config_t config
    ...
    device_load_config(context, framebuffer->loaded_configs, &config, (cl_uchar*)uniform_data);
