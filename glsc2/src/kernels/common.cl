...
inline uint local_scan_inclusive_add_ui(uint value, local volatile uint* l_temp) {
    uint id, num_ids, tmp;
    local volatile uint* ptr;

    #if DEVICE_SUB_GROUP_SUPPORT == 0
    id      = get_local_linear_id();
    num_ids = get_local_linear_size();
    #else
    id      = get_sub_group_id();
    num_ids = get_num_sub_groups();
    tmp     = sub_group_scan_inclusive_add_ui(value);
    value   = sub_group_broadcast_ui(tmp, get_sub_group_size()-1);
    #endif
    ptr     = &l_temp[id];
    *ptr    = value;
